...
Catalogs whose applications are open source throughout — both the code here and everything it deploys.

Nothing in this section requires an entitlement from anyone. If an application here turns out to pull something that is not open source, it belongs in [`contrib/`](../contrib/) instead, and moving it is a bug fix, not a policy change.

This catalog is CozyLLM's open-source AI stack: model serving, API gateways, notebooks, visual pipelines, image generation, and an opt-in SRE assistant. Every app appears as a first-class entity in the Cozystack dashboard — deploy with a click, configure through a generated form, scale and tear down independently.
...
### Looking for n8n or Open WebUI?

Two more apps — **n8n** (workflow automation) and **Open WebUI** (chat UI) — live in the [`contrib/`](../contrib/) catalog, kept separate because their upstream projects are distributed under licenses that are not OSI-approved open source (n8n's Sustainable Use License, the Open WebUI License). Install the contrib catalog alongside this one if you need those apps and your use complies with the vendors' terms.
